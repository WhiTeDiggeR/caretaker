...
func _build_manual_evacuation_stair() -> void:
	_add_shaft_shell("ManualEvacShaft", -55.0, -40.0, 45.0, 67.5, 0.0, 16.0, false)
	_add_ramp("ManualEvacUpper", Vector3(-50.0, 12.0, 40.0), Vector3(-50.0, 6.0, 62.5))
	_add_landing("ManualEvacLanding", Vector3(-47.5, 6.0, 62.5), Vector2(10.0, 8.0))
	_add_ramp("ManualEvacLower", Vector3(-45.0, 6.0, 62.5), Vector3(-50.0, 0.0, 40.0))
...
func _add_ramp(name_value: String, start: Vector3, end: Vector3) -> void:
	...
	_add_mesh_and_collision(
		body,
		"LeftRail",
		Vector3(0.25, 1.4, length),
		Vector3(-RAMP_WIDTH * 0.5 + 0.12, 0.95, 0.0),
		WALL_MATERIAL
	)
	_add_mesh_and_collision(
		body,
