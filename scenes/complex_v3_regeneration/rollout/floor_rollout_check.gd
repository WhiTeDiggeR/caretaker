...
const VISUAL_SECTORS := {
	"lower": ["L-OLD-CORE", "L-CHAMBER-2", "L-OLD-RECEIVING"],
	"technical": ["T-UTILITIES", "T-EAST-VERTICAL", "T-OLD-ACCESS"],
}
...
func _run() -> void:
	for argument: String in OS.get_cmdline_user_args():
		if argument.begins_with("--level="):
			_level = argument.trim_prefix("--level=")
	if _level not in ["lower", "technical"]:
		push_error("unsupported floor rollout level: %s" % _level)
		quit(1)
		return
	var manifest := _load_json("res://tools/complex_v3_regeneration/rollouts/%s_generation_manifest.json" % _level)
	...
	for value: Variant in manifest.get("sectors", []):
		...
		expected_objects += (dressing_data.get("placements", []) as Array).size()
	...
	if object_count != expected_objects:
		_errors.append("loaded %d authored objects, expected %d" % [object_count, expected_objects])
	if _errors.is_empty() and OS.get_cmdline_user_args().has("--visual"):
		await _render_comparisons(manifest, dressing_by_sector)
	if _errors.is_empty():
		print("FLOOR_ROLLOUT_GODOT_OK level=%s sectors=%d authored_objects=%d" % [_level, loaded, object_count])
	else:
...
func _validate_shapes(node: Node, sector_id: String) -> void:
	if node is MeshInstance3D and (node as MeshInstance3D).mesh == null:
		_errors.append("empty mesh in %s: %s" % [sector_id, node.get_path()])
...
func _render_comparisons(manifest: Dictionary, dressing_by_sector: Dictionary) -> void:
	...
	var output := "res://scenes/complex_v3_regeneration/rollout/%s/reports/visuals" % _level
	...
	for sector_value: Variant in VISUAL_SECTORS[_level]:
		var sector_id := str(sector_value)
		...
		var floor_y := -6.0 if _level == "lower" else -11.5
		var center := Vector3((float(sector_bounds[0]) + float(sector_bounds[2])) * 0.5, floor_y, (float(sector_bounds[1]) + float(sector_bounds[3])) * 0.5)
		...
		for phase: String in ["before", "after"]:
			...
			_add_lighting(world)
			if phase == "before":
				var zone := (load(str((dressing_by_sector[sector_id] as Dictionary)["zone_scene"])) as PackedScene).instantiate() as ComplexV3BlockoutPart
				...
				world.add_child((load(str((dressing_by_sector[sector_id] as Dictionary)["dressing_scene"])) as PackedScene).instantiate())
			...
			if root.get_texture().get_image().save_png(path) != OK:
				...
				print("FLOOR_ROLLOUT_VIEW %s" % ProjectSettings.globalize_path(path))
			world.free()
			await process_frame
...
func _hide_ceilings(node: Node) -> void:
	...
	for child: Node in node.get_children(): _hide_ceilings(child)
