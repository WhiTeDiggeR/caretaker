...
- `complex_v3_visual_check.gd` — воспроизводимые контрольные рендеры по одной зоне каждого уровня в `user://complex_v3_sector_captures`.
...
При открытии отдельной сцены из `zones/` её `editor_preview_enabled` строит временный узел `Generated` прямо в 3D-вьюпорте. Preview не сохраняется в `.tscn`, не содержит physics collision и может быть отключён в Inspector; `AuthoredContent` при перестроении не изменяется. В полной сборке дочерние preview автоматически отключены.
